     ...
     (atacante,
         if (nuevoAtacado.tieneCola) {
           nuevoAtacado.bajarKiHasta(1).tipo match {
             case Saiyajin(MonoSaiyajin,true)
             => nuevoAtacado.cambiarTipo(Saiyajin(NormalSaiyajin,false)).cambiarEstado(Inconsciente)
             case Saiyajin(forma,true)
             => nuevoAtacado.cambiarTipo(Saiyajin(forma,false))
             case _ => nuevoAtacado
           }
         } else nuevoAtacado
     )
